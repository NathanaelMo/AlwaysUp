...
func _ready():
	if not Engine.is_editor_hint():
		# Assurez-vous que tous les nœuds nécessaires existent
		if not has_node("Camera"):
			print("Camera node not found. Creating one.")
...
func _physics_process(_delta):
	if not is_instance_valid(_camera):
		print("Camera is not valid. Skipping _physics_process.")
		return
...
func _update_camera_properties() :
	if not is_instance_valid(_camera):
		print("Camera is not valid. Skipping property update.")
		return
	
	_camera.keep_aspect = keep_aspect as Camera3D.KeepAspect
