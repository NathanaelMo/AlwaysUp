...
func sample_hemisphere(rng: RandomNumberGenerator, radius: float):
	# Générer une position uniquement dans l'hémisphère supérieur
	while true:
		var pos = Vector3(
			rng.randf_range(-1.0, 1.0),
			rng.randf_range(0.0, 1.0),  # Y uniquement positif
			rng.randf_range(-1.0, 1.0)
		)
		if pos.length_squared() <= 1.0:
			# Déplacer les étoiles plus haut pour éviter qu'elles soient trop proches de l'horizon
			pos.y += 0.2  # Ajout d'un décalage vers le haut
			pos = pos.normalized()  # Normaliser pour garder les étoiles sur la sphère
			return pos * radius
...
func _process(delta):
	...
	if generate_at_origin:
		# Place the origin star high in the sky instead of at 0,0,0
		var origin_pos = Vector3(0, size * 0.8, 0)
		stars.push_back(class_G.get_star(origin_pos, 0.5))
	...
	for i in range(0, star_count):
		...
		stars.push_back(category.get_star(sample_hemisphere(rng, size), rng.randf()))
